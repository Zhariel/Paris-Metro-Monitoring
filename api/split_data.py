    ...
    for i in lis:
        ...
        if (i["type"] == "public_transport"):
            ...
            info = {
                ...
                "from"                  : i["from"]["stop_point"]["name"] if ("stop_point" in i["from"]) else  i["from"]["stop_area"]["name"] if ("stop_area" in i["from"]) else  i["from"]["address"]["name"] if ("address" in i["from"]) else i["from"]["name"],
                "to"                    : i["to"]["stop_point"]["name"] if ("stop_point" in i["to"]) else  i["to"]["stop_area"]["name"] if ("stop_area" in i["to"]) else  i["to"]["address"]["name"] if ("address" in i["to"]) else i["to"]["name"],
                "stops"                 : stops
            }
        elif (i["type"] == "street_network"):
            mode = "walking"
            info = {
                "code"                  : "",
                "duration"              : i["duration"] / 60,
...
def format_itinararies(lis):
    ...
    column_names    = ["id", "nb_transfers", "walking_duration", "arrival_date_time", "departure_date_time", "fare_value", "co2_emission_value", "type", "duration", "nb_sections","sections"]
    df              = pd.DataFrame(columns = column_names)
    j               = 0
    for i in lis:
        print("journey" + str(j))
        ...
        df2                 = {'id': id, 'nb_transfers': nb_transfers, 'walking_duration': walking_duration, 'arrival_date_time' : arrival_date_time, "departure_date_time" : departure_date_time, "fare_value": fare_value,
                       "co2_emission_value" : co2_emission_value, "type" : type, "duration" : duration, "nb_sections" : nb_sections, "sections" : sections}
        df                  = df.append(df2, ignore_index = True)
        j                   = j + 1
